    ...
    $.get(productListApiUrl, function (response) {
        ...
        if(response) {
            ...
            $.each(response, function(index, product) {
                options += '<option value="'+ product.pid +'">'+ product.pname +'</option>';
                productPrices[product.pid] = product.price_per_unit;
            });
...
$("#saveOrder").on("click", function(){
    ...
    for(var i=0;i<formData.length;++i) {
        ...
        switch(element.name) {
            case 'customerName':
                requestPayload.cname = element.value;
                break;
